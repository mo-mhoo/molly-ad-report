...
if df_curr is not None and not df_curr.empty:
    if platform_sel == "Google":
        ...
        for metric, style, hib in [
            ...
            rows.append(row)
        st.html(pd.DataFrame(rows).to_html(escape=False, index=False))
        ...
        st.html(build_table_html(curr_m, comp_m, mom_m, yoy_m))
    ...
    if data_source == "Meta API 自動抓取" and platform_sel == "Meta":
        ...
        if df_ads_raw is not None and not df_ads_raw.empty:
            ...
            for dim_col, label in [
                ...
                if not tbl.empty:
                    st.html(tbl.to_html(escape=False, index=False))
                else:
                    st.caption("無資料")
                st.markdown("")
