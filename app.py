...
def create_budget_schedule(access_token, campaign_id, time_start, time_end, pct_increase):
    ...
    if "Invalid parameter" in result.get("error", {}).get("message", "") and result.get("error", {}).get("error_subcode") not in (3858199, 3858175, 3858090, 3858119):
        print(f"[DEBUG] campaign Invalid parameter camp={campaign_id} err={result.get('error')}")
        _bi = requests.get(
            f"https://graph.facebook.com/v25.0/{campaign_id}",
            params={"fields": "daily_budget,lifetime_budget", "access_token": access_token},
            timeout=15,
        ).json()
    ...
    with col1:
        ...
        if preset != _prev_preset:
            ...
            if preset == "今日":
                ...
                st.session_state["api_curr_s"] = date(_last_month_end.year, _last_month_end.month, 1)
                st.session_state["api_curr_e"] = _last_month_end
