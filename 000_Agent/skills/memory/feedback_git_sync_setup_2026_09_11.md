...
另外查到一個目前沒處理的旁支：Google Drive 上有一份 2026-09-10 的 `~/.claude/` 完整快照備份（flat file 命名如 `claude-skills__memory__xxx.md`），裡面有幾個這個專案 `000_Agent/skills/memory/MEMORY.md` 索引裡沒有的記憶檔名（例如 `project_pmax_flight_autopause.md`、`feedback_mh_report_compile_full_after_organic.md`），看命名可能屬於 [[project_mh_ai_agent_setup]] 那個獨立的 MH 專案、也可能是這個專案漏收的，還沒有比對清楚，之後有空要花時間對一次，不要假設兩邊已經一致。

**已發生一次的具體案例（2026-09-11）**：Molly 在 Mac 上發現 `meta-ad-video-img-upload` skill 不存在，但這台 Mac 的 git 本來就顯示「up to date with origin/main」——代表這個 skill 根本不是「沒 pull」的問題，而是它 2026-08-31 在 Windows 上建立時，git 同步機制還沒修好（要到 2026-09-11 才修好），所以從來沒被 commit 過，Windows 那邊本機有、GitHub 沒有。靠 Google Drive 上的 `~/.claude` 快照備份（`meta-ad-video-img-upload_SKILL.md`、`project_meta_ad_video_img_upload_skill.md`）救回來，重建進這台 Mac 的 `000_Agent/skills/meta-ad-video-img-upload/SKILL.md` 並 commit+push，這樣以後才不會再遺失。**這代表 2026-08-31～2026-09-11 之間在 Windows 上建立/修改、但 Windows 那邊還沒手動同步的其他 skill/記憶檔，理論上同樣有這個缺口**——之後如果又發現某個 skill/記憶在 Mac 上「應該有卻沒有」，先假設是這個已知缺口，去 Google Drive 的 `~/.claude` 快照備份找，不用當成新的怪異問題重新排查。
