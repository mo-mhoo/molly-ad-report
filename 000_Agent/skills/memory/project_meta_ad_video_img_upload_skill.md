...
**momo CPAS 精選集(Canvas)廣告的creative結構完整欄位（2026-09-11 修復確認）**：momo TOP3 魚油5入等 Canvas 廣告，Ads Manager 顯示「格式」正確變成「精選集」、「目的地」出現「即時體驗」，需要 creative 同時帶齊 `product_set_id`、`retailer_item_ids`、`post_click_configuration` 三個欄位——缺任何一個都會導致 Ads Manager 判斷不出這是精選集結構，顯示異常（例如格式沒顯示精選集、目的地沒秀出即時體驗、或出現警告角標）。修好後畫面下方會多一個「目的地貼文即時體驗」的進階選項（要不要選「高效速成＋目的地」），這是格式正確辨識成精選集後才會出現的進階選項，不是必要欄位，跟前面歷史廣告用同一套邏輯處理即可。**已知這個具體修復是在另一個 session 裡完成的，這則記憶是事後補記，不是原本修復時當場寫的**——參見 [[feedback_immediate_memory_write]] 這條關於「修完/更新完要立刻寫記憶，不能事後才想起來」的教訓。

相關：[[project_drive_video_to_meta_upload_pipeline]]、[[project_meta_collection_canvas_video_creation]]、[[project_meta_ad_launch_check_skill]]、[[feedback_sheets_emoji_mojibake]]、[[project_meta_cpas_app_dev_mode_blocker]]、[[meta-ad-copy]]、[[project_maohai_official_campaign_structure]]、[[feedback_immediate_memory_write]]
