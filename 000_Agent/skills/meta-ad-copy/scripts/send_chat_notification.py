#!/usr/bin/env python3
"""推播 meta-ad-copy 上稿結果到 Google Chat（讀 .env 的 GOOGLE_CHAT_WEBHOOK）。
...
def load_webhook() -> str:
    if os.environ.get("GOOGLE_CHAT_WEBHOOK"):
        return os.environ["GOOGLE_CHAT_WEBHOOK"]
    if ENV_PATH.exists():
        for line in ENV_PATH.read_text(encoding="utf-8").splitlines():
            line = line.strip()
            if line.startswith("GOOGLE_CHAT_WEBHOOK="):
...
def main() -> int:
    ...
    if not webhook:
        print("錯誤：找不到 GOOGLE_CHAT_WEBHOOK（.env 或環境變數皆無值），已中止，未送出。", file=sys.stderr)
        return 1
