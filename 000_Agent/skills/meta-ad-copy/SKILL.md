...
這個腳本會讀 `.env` 裡的 `GOOGLE_CHAT_WEBHOOK`（已經設定好，不用問 Molly 要不要存 `.env`），從不會把 webhook 網址本身印出來。如果 Molly 說之後這個問題不用每次都問（例如她想改成跟走期排程一樣「預設就推、不用每次確認」），要等她真的這樣明確說了才調整成不問直接送，在那之前維持問她。
